...
function handle_input()
  ...
  if as_gamepad then
    key_up = gamepad:isGamepadDown("dpup")
    key_down = gamepad:isGamepadDown("dpdown")
    key_left = gamepad:isGamepadDown("dpleft")
    key_right = gamepad:isGamepadDown("dpright")
    key_shoot = gamepad:isGamepadDown("a")
    key_trow_nuclear_reactor = gamepad:isGamepadDown("b")

    spaceship_speed_x = spaceship_speed_x + gamepad:getGamepadAxis("leftx")
    spaceship_speed_y = spaceship_speed_y - gamepad:getGamepadAxis("lefty")
  end
...
function love.load(arg)
  ...
  asset_background_stars = love.graphics.newImage("/assets/background_stars.png")
...
function love.draw()
  ...
  if not is_game_finish then
    ...
    love.graphics.draw(asset_background_stars, 0, math.floor(background_location * 1.5) % love.graphics.getHeight() - love.graphics.getHeight(), 0, width_scale_factor, height_scale_factor)
    love.graphics.draw(asset_background_stars, 0, math.floor(background_location * 1.5) % love.graphics.getHeight(), 0, width_scale_factor, height_scale_factor)
...
function draw_spaceship()
  love.graphics.draw(asset_spaceship, spaceship_x + math.random(0, spaceship_speed_y) / 5, love.graphics.getHeight() - spaceship_height - 16, 0, 2, 2 - (spaceship_speed_y / spaceship_max_speed) / 10)
  love.graphics.setColor( 255, 255, 255, 255 * (spaceship_speed_y / spaceship_max_speed))
  love.graphics.draw(asset_truster, spaceship_x + math.random(0, spaceship_speed_y) / 5, love.graphics.getHeight() - spaceship_height - 16, 0, 2, 2 - (spaceship_speed_y / spaceship_max_speed) / 10)
  love.graphics.setColor( 255, 255, 255, spaceship_takedamages)
  love.graphics.draw(asset_shield, spaceship_x + math.random(0, spaceship_speed_y) / 5, love.graphics.getHeight() - spaceship_height - 16, 0, 2, 2 - (spaceship_speed_y / spaceship_max_speed) / 10)
...
function spaceship_shootbullet(x)
  if (shoot_cooldown == 0) then
    spaceship_bullet[spaceship_bullet_count] = {x = x, y = love.graphics.getHeight() - 32, speed = 10}
...
function update_ennemy(dt)
  for i,e in pairs(ennemies) do
    ...
    if CheckCollision(e["x"], e["y"], e["width"], e["height"], spaceship_x, love.graphics.getHeight() - spaceship_height, spaceship_width, spaceship_height) then
...
function soundmanager_play(source)
  src = love.audio.newSource(source)
  src:play()
